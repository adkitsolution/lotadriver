...
class DocumentController extends GetxController {
  ...
  Future<dynamic> uploadProfile() async {
    try {
      ...
      log('after upload document :$document');

      request.fields['driver_id'] = Preferences.getInt(Preferences.userId).toString();
      request.fields['documents'] = jsonEncode(document);
      log('after upload  request field  : ${request.fields}');
      log('after upload  request files : ${request.files}');

      var res = await request.send();
      var responseData = await res.stream.toBytes();
      log("after upload response : $responseData");
